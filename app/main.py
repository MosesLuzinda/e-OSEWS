...
def infer_rainfall_index_from_location(latitude: float, longitude: float) -> float | None:
    """
    Estimate normalized rainfall index (0..1) from recent precipitation.
    Uses Open-Meteo historical daily precipitation for the last 7 days.
    """
    try:
        ...
        req = request.Request(url, headers={"User-Agent": "e-OSEWS/1.0"})
        with request.urlopen(req, timeout=6) as res:
            payload = json.loads(res.read().decode("utf-8"))
        values = payload.get("daily", {}).get("precipitation_sum", [])
        rainfall_values = [float(v) for v in values if v is not None]
        if not rainfall_values:
            return None

        # Normalize 7-day rainfall sum with a practical cap of 200 mm.
        weekly_total = sum(rainfall_values)
        return round(min(max(weekly_total / 200.0, 0.0), 1.0), 3)
    except Exception:
        return None
...
def create_event():
    ...
    if normalized.get("rainfall_index") is None:
        lat = normalized.get("latitude")
        lon = normalized.get("longitude")
        if lat is not None and lon is not None:
            inferred_rainfall = infer_rainfall_index_from_location(float(lat), float(lon))
            if inferred_rainfall is not None:
                normalized["rainfall_index"] = inferred_rainfall
...
@app.get("/briefings/training")
def briefing_training():
    return render_template(
        "site/briefing_item.html",
        current_page="resources",
        page_title="Integrated early warning training",
        category="News",
        title="Integrated early warning training for district focal persons",
        summary="Hands-on workshops train focal persons on dashboard use, structured reporting, and response workflow.",
        bullets=[
            "Dashboard and mobile workflow orientation for district, facility, and community teams.",
            "Structured case reporting with GPS capture, syndrome selection, and exposure flags.",
            "Alert lifecycle practice: triage, investigation, escalation, and closure with audit trail.",
            "Mentor-of-trainers model established in each pilot district.",
        ],
    )


@app.get("/briefings/seasonal-advisory")
def briefing_seasonal_advisory():
    return render_template(
        "site/briefing_item.html",
        current_page="resources",
        page_title="Seasonal RVF risk advisory",
        category="Alert",
        title="Seasonal RVF risk advisory",
        summary="Recent rainfall and vegetation anomalies indicate elevated RVF amplification risk over the coming 8-12 weeks.",
        bullets=[
            "Prioritize livestock vaccination in flagged parishes.",
            "Pre-position PPE and referral kits at district facilities.",
            "Expand community sensitization on safe slaughter and milk handling.",
            "Increase active surveillance cadence for abortion clusters and febrile syndromes.",
        ],
    )


@app.get("/briefings/action-framework")
def briefing_action_framework():
    return render_template(
        "site/briefing_item.html",
        current_page="resources",
        page_title="Rift Valley fever action framework",
        category="Publication",
        title="Rift Valley fever action framework",
        summary="Guidance for decision makers on prevention, response, and recovery actions across the RVF risk continuum.",
        bullets=[
            "Anticipation: thresholds, pre-positioning, and readiness planning.",
            "Detection: case definitions, reporting, and confirmation pathways.",
            "Response: coordinated animal-human health operations and risk communication.",
            "Recovery: after-action review and district resilience strengthening.",
        ],
    )


@app.get("/briefings/innovation")
def briefing_innovation():
    return render_template(
        "site/briefing_item.html",
        current_page="resources",
        page_title="Driving preparedness through innovation",
        category="Brochure",
        title="Driving preparedness through innovation",
        summary="How e-OSEWS supports anticipatory action through offline-first reporting, analytics, and district-level coordination.",
        bullets=[
            "Mobile and USSD channels improve field signal timeliness.",
            "Environmental signals are combined with structured case reports.",
            "Risk scoring and workflow tools support prioritized response.",
            "A shared dashboard strengthens One Health collaboration.",
        ],
    )
